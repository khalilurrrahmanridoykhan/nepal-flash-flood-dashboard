...
export default function CinematicScene({ progress, activeEvent }: Props) {
  ...
  const visibleCount = progress === 0 ? 1 : Math.max(2, Math.ceil(progress * (riverPoints.length - 1)) + 1); const full = riverPoints.map((point) => `${point.x},${point.y}`).join(" "); const travelled = riverPoints.slice(0, visibleCount).map((point) => `${point.x},${point.y}`).join(" ");
  return <div className="real-terrain" data-testid="cinematic-scene"><div ref={container} className="real-terrain-map" />
    {ready && <svg className="terrain-evidence" aria-label="Reconstructed flood corridor">
      <defs><linearGradient id="flood-water" x1="0" x2="1"><stop offset="0" stopColor="#4c3022" /><stop offset=".55" stopColor="#916039" /><stop offset="1" stopColor="#d1ad79" /></linearGradient><filter id="rough-water"><feTurbulence type="fractalNoise" baseFrequency=".018 .07" numOctaves="2" seed="7" result="noise" /><feDisplacementMap in="SourceGraphic" in2="noise" scale="7" /></filter></defs>
      <polyline className="terrain-corridor" points={full} /><polyline className="flood-bed" points={travelled} /><polyline className="terrain-flood" points={travelled} /><polyline className="flood-foam" points={travelled} />
