...
export default function CinematicScene({ progress, activeEvent }: Props) {
  const container = useRef<HTMLDivElement>(null); const mapRef = useRef<Map | null>(null); const [riverPoints, setRiverPoints] = useState<Point[]>([]); const [eventPoints, setEventPoints] = useState<Point[]>([]); const [ready, setReady] = useState(false);
  useEffect(() => {
    ...
    const project = () => { setRiverPoints(trisuliRiver.map((coordinate) => map.project(coordinate))); setEventPoints(riverCoordinates.map((coordinate) => map.project(coordinate))); };
    map.on("style.load", () => {
  ...
  const target = timelineEvents[activeEvent].coordinates; let nearestIndex = 0, nearestDistance = Infinity; trisuliRiver.forEach((coordinate, index) => { const distance = Math.hypot(coordinate[0] - target[0], coordinate[1] - target[1]); if (distance < nearestDistance) { nearestDistance = distance; nearestIndex = index; } }); const localFlood = progress === 0 ? "" : riverPoints.slice(Math.max(0, nearestIndex - 16), Math.min(riverPoints.length, nearestIndex + 4)).map((point) => `${point.x},${point.y}`).join(" ");
  return <div className="real-terrain" data-testid="cinematic-scene"><div ref={container} className="real-terrain-map" />
    {ready && <svg className="terrain-evidence" aria-label="Reported flood locations and local reconstructed flood corridor">
      <defs><linearGradient id="local-flood-water" x1="0" x2="1"><stop offset="0" stopColor="#365a57" /><stop offset=".48" stopColor="#7a6046" /><stop offset="1" stopColor="#b18a5e" /></linearGradient><filter id="local-rough-water"><feTurbulence type="fractalNoise" baseFrequency=".018 .075" numOctaves="2" seed="11" result="noise" /><feDisplacementMap in="SourceGraphic" in2="noise" scale="3" /></filter></defs>
      {localFlood && <><polyline className="flood-bed" points={localFlood} /><polyline className="terrain-flood local" points={localFlood} /><polyline className="flood-foam" points={localFlood} /></>}
