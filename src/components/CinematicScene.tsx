...
export default function CinematicScene({ progress: stageProgress, activeEvent }: Props) {
  const container = useRef<HTMLDivElement>(null); const mapRef = useRef<Map | null>(null); const previousEvent = useRef(0); const flowMarkers = useRef<maplibregl.Marker[]>([]); const [eventPoints, setEventPoints] = useState<Point[]>([]); const [ready, setReady] = useState(false);
  useEffect(() => {
  ...
  useEffect(() => { const map = mapRef.current, event = timelineEvents[activeEvent]; if (!map || !event) return; const priorIndex = previousEvent.current, previous = timelineEvents[Math.max(0, activeEvent - 1)].coordinates; previousEvent.current = activeEvent; const bearing = Math.atan2(event.coordinates[0] - previous[0], event.coordinates[1] - previous[1]) * 180 / Math.PI, camera = { center: event.coordinates, zoom: activeEvent === 0 ? 14.6 : 15.2, pitch: 48, bearing: Number.isFinite(bearing) && activeEvent > 0 ? bearing : -22 }; if (activeEvent < priorIndex) map.jumpTo(camera); else map.easeTo({ ...camera, duration: 1800, essential: true }); }, [activeEvent]);
  useEffect(() => {
    ...
    const markers: maplibregl.Marker[] = []; for (let index = 0; index < 24; index++) { const wrapper = document.createElement("span"), particle = document.createElement("span"); wrapper.className = "water-marker"; particle.className = "water-particle"; particle.style.animationDelay = `${-index * .07}s`; wrapper.appendChild(particle); markers.push(new maplibregl.Marker({ element: wrapper, anchor: "center" }).setLngLat(segment[0]).addTo(map)); } flowMarkers.current = markers;
