...
export default function CinematicScene({ progress: stageProgress, activeEvent }: Props) {
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({ container: host, style: terrainStyle, center: [85.25, 28.10], zoom: 10.35, pitch: 48, bearing: -12, maxPitch: 80, attributionControl: false, canvasContextAttributes: { antialias: true } });
  ...
  useEffect(() => {
    const map = mapRef.current; if (!map || !ready) return; flowMarkers.current.forEach((marker) => marker.remove()); flowMarkers.current = [];
    const markers: maplibregl.Marker[] = []; for (let index = 0; index < 72; index++) { const wrapper = document.createElement("span"), particle = document.createElement("span"); wrapper.className = "water-marker"; particle.className = "water-particle"; particle.style.animationDelay = `${-index * .045}s`; wrapper.appendChild(particle); markers.push(new maplibregl.Marker({ element: wrapper, anchor: "center" }).setLngLat(trisuliRiver[0]).addTo(map)); } flowMarkers.current = markers;
    let frame = 0, active = true; const started = performance.now(); const animate = (now: number) => { if (!active) return; const progress = stageProgressRef.current, totalMinutes = timelineEvents.at(-1)!.minutes, elapsedMinutes = progress * totalMinutes; let step = 0; while (step < timelineEvents.length - 2 && elapsedMinutes >= timelineEvents[step + 1].minutes) step++; const from = timelineEvents[step], to = timelineEvents[Math.min(timelineEvents.length - 1, step + 1)], local = Math.min(1, Math.max(0, (elapsedMinutes - from.minutes) / Math.max(1, to.minutes - from.minutes))), routeStart = eventRiverIndices[0], maxCursor = eventRiverIndices[step] + (eventRiverIndices[Math.min(eventRiverIndices.length - 1, step + 1)] - eventRiverIndices[step]) * local, covered = Math.max(0, maxCursor - routeStart), fullLength = Math.max(1, eventRiverIndices.at(-1)! - routeStart), visibleCount = Math.max(0, Math.ceil(covered / fullLength * markers.length)), drift = ((now - started) / 900) % 1; markers.forEach((marker, index) => { const element = marker.getElement(); if (index >= visibleCount || progress <= .002) { element.style.display = "none"; return; } element.style.display = "block"; const cursor = Math.min(maxCursor, routeStart + (index / Math.max(1, visibleCount - 1)) * covered + drift), base = Math.min(trisuliRiver.length - 2, Math.floor(cursor)), mix = cursor - Math.floor(cursor), a = trisuliRiver[base], b = trisuliRiver[base + 1]; marker.setLngLat([a[0] + (b[0] - a[0]) * mix, a[1] + (b[1] - a[1]) * mix]); }); frame = requestAnimationFrame(animate); }; frame = requestAnimationFrame(animate);
