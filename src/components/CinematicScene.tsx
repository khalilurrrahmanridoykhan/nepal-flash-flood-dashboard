...
export default function CinematicScene({ progress: stageProgress, activeEvent }: Props) {
  ...
  useEffect(() => {
    ...
    const updateFlowScale = () => { const detail = Math.max(0, Math.min(1, (map.getZoom() - 10) / 7)); host.style.setProperty("--flow-width", `${12 + detail * 48}px`); host.style.setProperty("--flow-height", `${5 + detail * 17}px`); host.style.setProperty("--flow-glow", `${2 + detail * 7}px`); }; const project = () => { setEventPoints(riverCoordinates.map((coordinate) => map.project(coordinate))); updateFlowScale(); };
    map.on("style.load", () => {
